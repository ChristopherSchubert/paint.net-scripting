...
#region UICode
FilenameControl Amount1 = @"C:\Users\Chris\Documents\Unity3D\Animal_Assets\Assets\StaticAssets\Textures\Flora\Trees\Leaves\Maps\ash_black_branch_1.png_ao.png"; // Path|png
TextboxControl Amount2 = "_m-ao-sm"; // [0,255] Output Postfix
ListBoxControl Amount3 = 1; // Preset|None|Bark_M_AO_Sm|Leaf_AO_Sm|Smoothness|MetallicRGB_SmoothnessA|AlbedoRGB_SmoothnessA
ListBoxControl Amount4 = 0; // Channel R: Map From|None|Ambient Occlusion|Diffuse|Height|Metallic|Normal|Roughness|Smoothness|Specularity
ListBoxControl Amount5 = 0; // Channel R: Extract Channel|RGB|R|G|B|A
CheckboxControl Amount6 = false; // [0,1] Channel R: Invert
ListBoxControl Amount7 = 0; // Channel G: Map From|None|Ambient Occlusion|Diffuse|Height|Metallic|Normal|Roughness|Smoothness|Specularity
ListBoxControl Amount8 = 0; // Channel G: Extract Channel|RGB|R|G|B|A
CheckboxControl Amount9 = false; // [0,1] Channel G: Invert
ListBoxControl Amount10 = 0; // Channel B: Map From|None|Ambient Occlusion|Diffuse|Height|Metallic|Normal|Roughness|Smoothness|Specularity
ListBoxControl Amount11 = 0; // Channel B: Extract Channel|RGB|R|G|B|A
CheckboxControl Amount12 = false; // [0,1] Channel B: Invert
ListBoxControl Amount13 = 0; // Channel A: Map From|None|Ambient Occlusion|Diffuse|Height|Metallic|Normal|Roughness|Smoothness|Specularity
ListBoxControl Amount14 = 0; // Channel A: Extract Channel|RGB|R|G|B|A
CheckboxControl Amount15 = false; // [0,1] Channel A: Invert
ListBoxControl Amount16 = 1; // Take Size From|RGB|R|G|B|A
CheckboxControl Amount17 = false; // [0,1] Process
#endregion
...
{ 
    ...
    if (
            (takeSizeFrom == ChannelType.R && mapFromR == TextureType.None) ||
            (takeSizeFrom == ChannelType.G && mapFromG == TextureType.None) ||
            (takeSizeFrom == ChannelType.B && mapFromB == TextureType.None) ||
            (takeSizeFrom == ChannelType.A && mapFromA == TextureType.None)
        )
    {
        Debug.WriteLine("Invalid size source.");
        return;
    }
    
    if (mapFromR == TextureType.None && mapFromG == TextureType.None && mapFromB == TextureType.None && mapFromA == TextureType.None)
    {
        Debug.WriteLine("No mapping specified.");
        return;
    }

    if (mapFromR == null || mapFromG == null || mapFromB == null || mapFromA == null)
    {
        Debug.WriteLine("Mapping failed.");
        return;
    }
    ...
    else
    {
        ...
        {
            Debug.WriteLine(string.Format("Path does not exist: {0}.", path));
            return;
        }
    }
    ...
    var sets = new Dictionary<string, MapFromFileSet>();
    ...
    foreach(var filePath in Directory.GetFiles(directory))
    {
        ...
        string pathSansPostfix = finalFilePath.Replace(".png", string.Empty);
        var textureType = TextureType.None;

        if (MapFromFileSet.AmbientOcclusionPostfixes.Any(pf => pathSansPostfix.ToLower().EndsWith(pf)))
        {
            textureType = TextureType.AmbientOcclusion;
            var postfix = MapFromFileSet.AmbientOcclusionPostfixes.First(pf => pathSansPostfix.ToLower().EndsWith(pf));
            pathSansPostfix = pathSansPostfix.Substring(0, pathSansPostfix.Length - postfix.Length);
        }
        else if (MapFromFileSet.DiffusePostfixes.Any(pf => pathSansPostfix.ToLower().EndsWith(pf)))
        {
            textureType = TextureType.Diffuse;
            var postfix = MapFromFileSet.DiffusePostfixes.First(pf => pathSansPostfix.ToLower().EndsWith(pf));
            pathSansPostfix = pathSansPostfix.Substring(0, pathSansPostfix.Length - postfix.Length);
        }
        else if (MapFromFileSet.HeightPostfixes.Any(pf => pathSansPostfix.ToLower().EndsWith(pf)))
        {
            textureType = TextureType.Height;
            var postfix = MapFromFileSet.HeightPostfixes.First(pf => pathSansPostfix.ToLower().EndsWith(pf));
            pathSansPostfix = pathSansPostfix.Substring(0, pathSansPostfix.Length - postfix.Length);
        }
        else if (MapFromFileSet.MetallicPostfixes.Any(pf => pathSansPostfix.ToLower().EndsWith(pf)))
        {
            textureType = TextureType.Metallic;
            var postfix = MapFromFileSet.MetallicPostfixes.First(pf => pathSansPostfix.ToLower().EndsWith(pf));
            pathSansPostfix = pathSansPostfix.Substring(0, pathSansPostfix.Length - postfix.Length);
        }
        else if (MapFromFileSet.NormalPostfixes.Any(pf => pathSansPostfix.ToLower().EndsWith(pf)))
        {
            textureType = TextureType.Normal;
            var postfix = MapFromFileSet.NormalPostfixes.First(pf => pathSansPostfix.ToLower().EndsWith(pf));
            pathSansPostfix = pathSansPostfix.Substring(0, pathSansPostfix.Length - postfix.Length);
        }
        else if (MapFromFileSet.RoughnessPostfixes.Any(pf => pathSansPostfix.ToLower().EndsWith(pf)))
        {
            textureType = TextureType.Roughness;
            var postfix = MapFromFileSet.RoughnessPostfixes.First(pf => pathSansPostfix.ToLower().EndsWith(pf));
            pathSansPostfix = pathSansPostfix.Substring(0, pathSansPostfix.Length - postfix.Length);
        }
        else if (MapFromFileSet.SmoothnessPostfixes.Any(pf => pathSansPostfix.ToLower().EndsWith(pf)))
        {
            textureType = TextureType.Smoothness;
            var postfix = MapFromFileSet.RoughnessPostfixes.First(pf => pathSansPostfix.ToLower().EndsWith(pf));
            pathSansPostfix = pathSansPostfix.Substring(0, pathSansPostfix.Length - postfix.Length);
        }
        else if (MapFromFileSet.SpecularityPostfixes.Any(pf => pathSansPostfix.ToLower().EndsWith(pf)))
        {
            textureType = TextureType.Specularity;
            var postfix = MapFromFileSet.SpecularityPostfixes.First(pf => pathSansPostfix.ToLower().EndsWith(pf));
            pathSansPostfix = pathSansPostfix.Substring(0, pathSansPostfix.Length - postfix.Length);
        }
        else
        {
        ...
        if (!sets.ContainsKey(pathSansPostfix))
        {
            sets.Add(pathSansPostfix, new MapFromFileSet(pathSansPostfix));
        }

        var set = sets[pathSansPostfix];

        set.SetTextureType(textureType, finalFilePath);
    }
    
    Debug.WriteLine("Listing sets...");

    var mappingIterations = new List<MappingIteration>();
    if (mapFromR.Value != TextureType.None) mappingIterations.Add(new MappingIteration() { textureType = mapFromR.Value, targetChannelType = ChannelType.R, extractChannelType = extractChannelR, invert = invertR });
    if (mapFromG.Value != TextureType.None) mappingIterations.Add(new MappingIteration() { textureType = mapFromG.Value, targetChannelType = ChannelType.G, extractChannelType = extractChannelG, invert = invertG });
    if (mapFromB.Value != TextureType.None) mappingIterations.Add(new MappingIteration() { textureType = mapFromB.Value, targetChannelType = ChannelType.B, extractChannelType = extractChannelB, invert = invertB });
    if (mapFromA.Value != TextureType.None) mappingIterations.Add(new MappingIteration() { textureType = mapFromA.Value, targetChannelType = ChannelType.A, extractChannelType = extractChannelA, invert = invertA });
            

    foreach(var mappingIteration in mappingIterations)
    { 
        Debug.WriteLine(mappingIteration.ToString());
    }
    ...
    Stream oStream = null;
    Surface oSurface = null;

    foreach (MapFromFileSet set in sets.Values)
    {
        ...
        try
        {
            var resetSurface = true;
            
            foreach (var iteration in mappingIterations)
            {
                if (IsCancelRequested) return;

                if (iteration.textureType == TextureType.None)
                {
                    throw new NotSupportedException("Bad iteration mapping.");
                }
    
                var filePath = set.GetTextureType(iteration.textureType);

                if (string.IsNullOrEmpty(filePath))
                {
                    throw new NotSupportedException("Bad file path");
                }

                iStream = File.Open(filePath, FileMode.Open); 
                iImage = Image.FromStream(iStream);
                iSurface = Surface.CopyFromGdipImage(iImage, false); 

                if (resetSurface)
                {
                    oSurface = new Surface(iSurface.Size);
                    resetSurface = false;
                }

                for (var y = 0; y < oSurface.Height; y++)
                for (var x = 0; x < oSurface.Width; x++)
                {
                    var input = iSurface[x,y];
                    var output = oSurface[x,y];

                    byte value = byte.MinValue;

                    switch(iteration.extractChannelType)
                    {
                        case ChannelType.RGB:
                        value = (byte)(int)((input.R + input.G + input.B) / 3);
                        break;
                        case ChannelType.R:
                        value = input.R;
                        break;
                    ...
                    switch(iteration.targetChannelType)
                    {
                        case ChannelType.R:
                        output.R = iteration.invert ? (byte)(byte.MaxValue - value) : value;
                        break;
                        case ChannelType.G:
                        output.G = iteration.invert ? (byte)(byte.MaxValue - value) : value;
                        break;
                        case ChannelType.B:
                        output.B = iteration.invert ? (byte)(byte.MaxValue - value) : value;
                        break;
                        case ChannelType.A:
                        output.A = iteration.invert ? (byte)(byte.MaxValue - value) : value;
                        break;
                    }
            ...
            var outputFile = string.Format("{0}{1}.png", set.prefix, outputPostfix);
            ...
            oStream = File.Open(outputFile, FileMode.Create);
        ...
        finally
        {
            ...
            if (iSurface != null) iSurface.Dispose();
            if (oStream != null) oStream.Close();
            if (oSurface != null) oSurface.Dispose();
        }
    }
}
...
{
    path.Value = (string)Amount1;
    outputPostfix.Value = (string)Amount2;
    presetType.Value=  (PresetType)(int)Amount3;
    mapFromR.Value = (TextureType)(int)Amount4;
    extractChannelR.Value = (ChannelType)(int)Amount5;
    invertR.Value = (bool)Amount6;
    mapFromG.Value = (TextureType)(int)Amount7;
    extractChannelG.Value = (ChannelType)(int)Amount8;
    invertG.Value = (bool)Amount9;
    mapFromB.Value = (TextureType)(int)Amount10;
    extractChannelB.Value = (ChannelType)(int)Amount11;
    invertB.Value = (bool)Amount12;
    mapFromA.Value = (TextureType)(int)Amount13;
    extractChannelA.Value = (ChannelType)(int)Amount14;
    invertA.Value = (bool)Amount15;
    takeSizeFrom.Value = (ChannelType)(int)Amount16;
    process.Value = (bool)Amount17;
}

private void AssignPreset(PresetType presetType)
{
    switch(presetType)
    {
        ...
        case PresetType.GenerateBark_M_AO_Sm:
            outputPostfix.Value = "_m-ao-sm";

            mapFromR.Value = TextureType.Metallic;
            extractChannelR.Value = ChannelType.RGB;
            invertR.Value = false;

            mapFromG.Value = TextureType.AmbientOcclusion;
            extractChannelG.Value = ChannelType.RGB;
            invertG.Value = false;

            mapFromB.Value = TextureType.None;
            extractChannelB.Value = ChannelType.RGB;
            invertB.Value = false;

            mapFromA.Value = TextureType.Roughness;
            extractChannelA.Value = ChannelType.RGB;
            invertA.Value = true;

            takeSizeFrom.Value = ChannelType.R;
            return;

        case PresetType.GenerateLeaf_AO_Sm:
            outputPostfix.Value = "_ao-sm";

            mapFromR.Value = TextureType.None;
            extractChannelR.Value = ChannelType.RGB;
            invertR.Value = false;
            ...
            takeSizeFrom.Value = ChannelType.G;
            return;

        case PresetType.MetallicRGB_SmoothnessA:
            outputPostfix.Value = "_m-sm";

            mapFromR.Value = TextureType.Metallic;
            extractChannelR.Value = ChannelType.R;
            invertR.Value = false;

            mapFromG.Value = TextureType.Metallic;
            extractChannelG.Value = ChannelType.G;
            invertG.Value = false;

            mapFromB.Value = TextureType.Metallic;
            extractChannelB.Value = ChannelType.B;
            invertB.Value = false;
        ...
        case PresetType.AlbedoRGB_SmoothnessA:
            outputPostfix.Value = "_a-sm";

            mapFromR.Value = TextureType.Diffuse;
            extractChannelR.Value = ChannelType.R;
            invertR.Value = false;

            mapFromG.Value = TextureType.Diffuse;
            extractChannelG.Value = ChannelType.G;
            invertG.Value = false;

            mapFromB.Value = TextureType.Diffuse;
            extractChannelB.Value = ChannelType.B;
            invertB.Value = false;
        ...
        default:
            throw new NotSupportedException(presetType.ToString());
...
{
    Amount1 = path.Value;
    Amount2 = outputPostfix.Value;
    Amount3 = (byte)(int)presetType.Value;
    Amount4 = (byte)(int)mapFromR.Value;
    Amount5 = (byte)(int)extractChannelR.Value;
    Amount6 = invertR.Value;
    Amount7 = (byte)(int)mapFromG.Value;
    Amount8 = (byte)(int)extractChannelG.Value;
    Amount9 = invertG.Value;
    Amount10 = (byte)(int)mapFromB.Value;
    Amount11 = (byte)(int)extractChannelB.Value;
    Amount12 = invertB.Value;
    Amount13 = (byte)(int)mapFromA.Value;
    Amount14 = (byte)(int)extractChannelA.Value;
    Amount15 = invertA.Value;
    Amount16 = (byte)(int)takeSizeFrom.Value;
    Amount17 = process.Value;
}

public enum PresetType
{
    None,
    GenerateBark_M_AO_Sm,
    GenerateLeaf_AO_Sm,
    GenerateSmoothness,
    MetallicRGB_SmoothnessA,
    AlbedoRGB_SmoothnessA
}

TrackingProperty<string> path = new TrackingProperty<string>();
TrackingProperty<string> outputPostfix = new TrackingProperty<string>();
TrackingProperty<PresetType> presetType = new TrackingProperty<PresetType>();
TrackingProperty<TextureType> mapFromR = new TrackingProperty<TextureType>();
TrackingProperty<ChannelType> extractChannelR = new TrackingProperty<ChannelType>();
TrackingProperty<bool> invertR = new TrackingProperty<bool>();
TrackingProperty<TextureType> mapFromG = new TrackingProperty<TextureType>();
TrackingProperty<ChannelType> extractChannelG = new TrackingProperty<ChannelType>();
TrackingProperty<bool> invertG = new TrackingProperty<bool>();
TrackingProperty<TextureType> mapFromB = new TrackingProperty<TextureType>();
TrackingProperty<ChannelType> extractChannelB = new TrackingProperty<ChannelType>();
TrackingProperty<bool> invertB = new TrackingProperty<bool>();
TrackingProperty<TextureType> mapFromA = new TrackingProperty<TextureType>();
TrackingProperty<ChannelType> extractChannelA = new TrackingProperty<ChannelType>();
TrackingProperty<bool> invertA = new TrackingProperty<bool>();
TrackingProperty<ChannelType> takeSizeFrom = new TrackingProperty<ChannelType>();
TrackingProperty<bool> process = new TrackingProperty<bool>();

public class MappingIteration
{
    public TextureType textureType;
    public ChannelType extractChannelType;
    public ChannelType targetChannelType;
    public bool invert;
    ...
    public override string ToString()
    {
        ...
        builder.AppendLine(string.Format("textureType: {0}", textureType));
        builder.AppendLine(string.Format("extractChannelType: {0}", extractChannelType));
        builder.AppendLine(string.Format("targetChannelType: {0}", targetChannelType));
        builder.AppendLine(string.Format("invert: {0}", invert));

        return builder.ToString();
    }

}
public class MapFromFileSet
{
    public MapFromFileSet(string prefix)
    {
        this.prefix = prefix;
    }

    public static string[] AmbientOcclusionPostfixes = new [] { "_ao", "_o", "_ambientocclusion", "_occ" };
    public static string[] DiffusePostfixes = new [] { "_d", "_a", "_diffuse", "_diff", "_alb", "_albedo" };
    public static string[] HeightPostfixes = new [] {"_h", "_hm", "_heightmap", "_height" };
    public static string[] MetallicPostfixes = new [] {"_m", "_metallic" };
    public static string[] NormalPostfixes = new [] {"_n", "_normal", "_nrm" };
    public static string[] RoughnessPostfixes = new [] {"_r", "_roughness" };
    public static string[] SmoothnessPostfixes = new [] {"_sm", "_smoothness", "_smooth"};
    public static string[] SpecularityPostfixes = new [] {"_sp", "_s", "_specularity" };
    ...
    public string GetTextureType(TextureType textureType)
    {
        ...
        switch(textureType)
        {
            ...
            case TextureType.Roughness:
                returnValue = roughness;
                break;
            case TextureType.Smoothness:
                returnValue = smoothness;
                break;
            case TextureType.Specularity:
                returnValue = specularity;
                break;
            default:
                throw new NotSupportedException(textureType.ToString());

        }

        if (string.IsNullOrWhiteSpace(returnValue))
        {
            throw new NotSupportedException(string.Format("No path available for {0}.", textureType));
        }

        return returnValue;
    }
    
    public void SetTextureType(TextureType textureType, string value)
    {
        switch(textureType)
        {
            case TextureType.None:
    ...
    public override string ToString()
    {
        ...
        builder.AppendLine(string.Format("AmbientOcclusion: {0}", ambientOcclusion == null ? "None" : ambientOcclusion));
        builder.AppendLine(string.Format("Diffuse: {0}", diffuse == null ? "None" : diffuse));
        builder.AppendLine(string.Format("Height: {0}", height == null ? "None" : height));
        builder.AppendLine(string.Format("Metallic: {0}", metallic == null ? "None" : metallic));
        builder.AppendLine(string.Format("Normal: {0}", normal == null ? "None" : normal));
        builder.AppendLine(string.Format("Roughness: {0}", roughness == null ? "None" : roughness));
        builder.AppendLine(string.Format("Smoothness: {0}", smoothness == null ? "None" : smoothness));
        builder.AppendLine(string.Format("Specularity: {0}", specularity == null ? "None" : specularity));
...
{
    None = 0, AmbientOcclusion = 1, Diffuse = 2, Height = 3, Metallic = 4, Normal = 5, Roughness = 6, Smoothness = 7, Specularity = 8 
}
...
/*
TextboxControl Amount2 = "_m_ao_sm"; // [0,255] Output Postfix
ListBoxControl Amount4 = 3; // Channel R: Map From|None|Diffuse|Height|Metallic|Normal|Roughness|Specularity|Ambient Occlusion
ListBoxControl Amount5 = 0; // Channel R: Extract Channel|RGB|R|G|B|A
CheckboxControl Amount6 = false; // [0,1] Channel R: Invert
*/
