...
{
    AntiAlias(working, src, selection, 3, 2, 1); 
...
{
    int blendDistance = 2*2*2*2*2*2*2;
    ...
    for (int y = selection.Top; y < selection.Bottom; y++)
    {
        ...
        PixelRef first = null;
        PixelRef second = null;

        if (IsAlphaNeutral(working[selection.Left, y]))
        {
            first = new PixelRef(selection.Left, y, averageColor);
        }

        for (int x = first == null ? 0 : 1; x < selection.Right; x++)
        {
            ColorBgra work = *workPtr;

            if (IsAlphaNeutral(work))
            {
                if (x == selection.Right-1)
                {
                    if (first != null)
                    {
                        var end = first.x + blendDistance > x ? x : first.x + blendDistance;

                        for(var sx = first.x; sx <= end; sx++)
                        {
                            var time = FloatUtil.Clamp((((float)sx - first.x) / ((float)(end - first.x))), 0, 1);

                            var diffB = averageColor.B - first.color.B;
                            var diffG = averageColor.G - first.color.G;
                            var diffR = averageColor.R - first.color.R;

                            var finalB = (byte)(int)(first.color.B + (time * diffB));
                            var finalG = (byte)(int)(first.color.G + (time * diffG));
                            var finalR = (byte)(int)(first.color.R + (time * diffR));

                            var color = ColorBgra.FromBgr(finalB, finalG, finalR);
                ...
                else
                {
                    if (first == null)
                    {
                        first = new PixelRef(x, y, working[x-2,y]);
                    }
                    else
                    {
                        if (x == selection.Right-1)
                        {
                            second = new PixelRef(x, y, averageColor);
                        }
                        else
                        {
                            second = new PixelRef(x, y, working[x+1,y]);
                        }
                    }
                }
            }
            else
            {
                if (second != null)
                {
                    var start = second.x - blendDistance < first.x ? first.x : second.x - blendDistance;

                    for(var sx = start; sx <= second.x; sx++)
                    {
                        var time = FloatUtil.Clamp((((float)sx - start) / ((float)(second.x - start))), 0, 1);

                        var diffB = second.color.B - first.color.B;
                        var diffG = second.color.G - first.color.G;
                        var diffR = second.color.R - first.color.R;
                ...
                {
                    working2[first.x, first.y] = first.color;
                    first = null;
                }
            }
    ...
    for (int x = selection.Left; x < selection.Right; x++)
    {
        ...
        if (IsAlphaNeutral(working[x, selection.Top]))
        {
            first = new PixelRef(x, selection.Top, averageColor);
        }

        for (int y = first == null ? 0 : 1; y < selection.Bottom; y++)
        {
            ColorBgra work = working[x,y];
            ...
            if (IsAlphaNeutral(work))
            {
                if (y == selection.Bottom-1)
                {
                    if (first != null)
                    {
                        var end = first.y + blendDistance > y ? y : first.y + blendDistance;

                        for(var sy = first.y; sy <= end; sy++)
                        {
                            var time = FloatUtil.Clamp((((float)sy - first.y) / ((float)(end - first.y))), 0, 1);
                ...
                else
                {
                    if (first == null)
                    {
                        first = new PixelRef(x, y, working[x,y-2]);
                    }
                    else
                    {
                        if (y == selection.Bottom-1)
                        ...
                        {
                            second = new PixelRef(x, y, working[x,y+1]);
                        }
                    }
                }
            }
            else
            {
                if (second != null)
                {
                    var start = second.y - blendDistance < first.y ? first.y : second.y - blendDistance;

                    for(var sy = start; sy <= second.y; sy++)
                    {
                        var time = FloatUtil.Clamp((((float)sy - start) / ((float)(second.y - start))), 0, 1);
                ...
                {
                    working3[first.x, first.y] = first.color;
                    first = null;
                }
            }
        }
    ...
    GaussianBlur(working2, working2, selection, 10);
    GaussianBlur(working3, working3, selection, 10);

    for (int y = selection.Top; y < selection.Bottom; y++)
    {
        ColorBgra* work2Ptr = working2.GetPointAddressUnchecked(selection.Left, y);
        ColorBgra* work3Ptr = working3.GetPointAddressUnchecked(selection.Left, y);

        for (int x = selection.Left; x < selection.Right; x++)
        {
            ColorBgra work2 = *work2Ptr;
            ColorBgra work3 = *work3Ptr;

            if (!IsAlphaNeutral(work2) && !IsAlphaNeutral(work3))
            {
                work2.A = byte.MaxValue;
                work3.A = byte.MaxValue;
                *work2Ptr = BlendedPixel(work2, work3, BlendModes.Average);
            }
            else if (!IsAlphaNeutral(work2))
            {
                work2.A = byte.MaxValue;
                *work2Ptr = work2;
            }
            else if (!IsAlphaNeutral(work3))
            {
                work3.A = byte.MaxValue;
                *work2Ptr = work3;
            }
    ...
    for (int y = selection.Top; y < selection.Bottom; y++)
    {
        ...
        for (int x = selection.Left; x < selection.Right; x++)
        {
            ...
            if (IsAlphaNeutral(work))
            {
                if (!IsAlphaNeutral(work2))
                {
                    work2.A = byte.MinValue;
                    *workPtr = work2;
                }
                else
                {
                    *workPtr = averageColor;
                }
            }
...
private bool IsAlphaNeutral(ColorBgra color)
{
    return color == ColorBgra.Transparent || color == ColorBgra.TransparentBlack;
}
...
{
    public PixelRef(int x, int y, ColorBgra color)
    {
        this.x = x;
        this.y = y;
        this.color = color;
