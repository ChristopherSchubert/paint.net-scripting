...
CheckboxControl Amount7 = true; // [0,1] Force Opaque
#endregion
...
unsafe void RenderInternal(Surface dst, Surface src)
{
    for (int y = selection.Top; y < selection.Bottom; y++)
    {
        for (int x = selection.Left; x < selection.Right; x++)
        {
            ...
            var analyzeColors = new List<ColorBgra>();

            if (x > 0) { analyzeColors.Add(working[x-1,y]); }
            if (x < width-1) { analyzeColors.Add(working[x+1,y]); }
            if (y > 0) { analyzeColors.Add(working[x,y-1]); }
            if (y < height-1) { analyzeColors.Add(working[x,y+1]); }
            if (x > 0 && y > 0) { analyzeColors.Add(working[x-1,y-1]); }
            if (x > 0 && y < height-1) { analyzeColors.Add(working[x-1,y+1]); }
            if (x < width-1 && y > 0) { analyzeColors.Add(working[x+1,y-1]); }
            if (x < width-1 && y < height-1) { analyzeColors.Add(working[x+1,y+1]); }
            
            var index = 0;

            foreach(var analyzeColor in analyzeColors)
            {
                if (analyzeColor.A == 0) transparentCount += 1;
                if (analyzeColor.A >= maxA) { maxA = analyzeColor.A; maxIndex = index; }
                if (analyzeColor.A <= minA) { minA = analyzeColor.A; minIndex = index; }
            ...
            if (minA == maxA)
            {
                if (workingColor.A == maxA)
                {
                    //workingColor = ColorBgra.Blue;
            ...
            else
            {
                ...
                {
                    analyzeColors.RemoveAt(maxIndex);
                    analyzeColors.RemoveAt(minIndex);
                }
                else 
                {
                    analyzeColors.RemoveAt(minIndex);
            ...
            var avgA = sumA / analyzeColors.Count;
            var avgR= sumR / analyzeColors.Count;
            var avgG = sumG / analyzeColors.Count;
            var avgB = sumB / analyzeColors.Count;
            
            if (transparentCount > 2 || avgA < 127)
            {
                //workingColor = ColorBgra.Green;
                working[x,y] = workingColor;
                continue;
            }
            else 
            {
                var skippedMax = false;
                var skippedMin = false;

                double variation = 0;

                foreach( var analysisColor in analyzeColors)
                {
                    variation += SquaredDifference(analysisColor.A, avgA);
                }
                 
                var stdDev = Math.Sqrt(variation);

                if (Math.Abs(workingColor.A-avgA) > stdDev)
                {
                    workingColor.A = (byte)avgA;
                    workingColor.R = (byte)avgR;
                    workingColor.G = (byte)avgG;
                    workingColor.B = (byte)avgB;
                    //workingColor = ColorBgra.Magenta;
                    working[x,y] = workingColor;
                    continue;
                }
                else 
                {
                    //workingColor = ColorBgra.Yellow;
                    working[x,y] = workingColor;
                    continue;
                }
            }
...
private double SquaredDifference(int value, double avg)
{
...
TrackingProperty<bool> forceOpaque = new TrackingProperty<bool>();
