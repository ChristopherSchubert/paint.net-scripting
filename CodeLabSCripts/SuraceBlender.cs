...
unsafe void PreRenderInternal(Surface src)
{
    ...
    Rotate(working, working2, selection, 180, 0, 0);

    BlendSurfaces(working2, working, working2, byte.MaxValue, BlendModes.SignedDifference);
    BlendSurfaces(working, working, working2, byte.MaxValue, BlendModes.GrainMerge);

    BlendSurfaces(working, src, working, (byte)127, BlendModes.Normal);
...
void Rotate(Surface dst, Surface src, Rectangle rect, double rotateX, double rotateY, double rotateZ)
{
    if (IsCancelRequested) return;
    ...
    try
    {
        ...
        rotateEffectConfigToken.SetPropertyValue(RotateZoomEffect.PropertyNames.RollAndRotate, new Tuple<double,double,double>(rotateX, rotateY, rotateZ));
