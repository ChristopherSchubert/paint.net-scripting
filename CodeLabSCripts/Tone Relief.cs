...
{ 
    ...
    var stagingSurface = new Surface(src.Size);
    ...
    for(var postProcessingSegmentIndex = 0; postProcessingSegmentIndex < postProcessingSegments; postProcessingSegmentIndex++)
    {
        ...
        var toneSurface = GetSurfaceFromToneBand(src, Convert.ToInt32(segmentLowToneBand), Convert.ToInt32(segmentHighToneBand));

        OilPainting(toneSurface, toneSurface, selection, 1, 50);
        
        var reliefSurface = new Surface(src.Size);
        Relief(reliefSurface, toneSurface, selection, angle);

        ColorSwap(reliefSurface, reliefSurface, ColorBgra.Black ,ColorBgra.Transparent);

        BlendSurfaces(toneSurface, toneSurface, reliefSurface, new BlendModes[] { reliefBlendMode });

        ColorSwap(toneSurface, toneSurface, ColorBgra.Black ,ColorBgra.Transparent);

        BlendSurfaces(stagingSurface, stagingSurface, toneSurface, new BlendModes[] { BlendModes.Normal });
    } 

    working.CopySurface(stagingSurface);
}
...
private void AssignUIValues()
{
    ...
    postProcessingSegments.Value = Amount5;
    postProcessingDarkIsLower.Value = Amount6;
    reliefBlendMode.Value = (BlendModes)(int)Amount7;
}
