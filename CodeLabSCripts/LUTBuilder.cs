...
FilenameControl Amount5 = @"C:\Users\Chris\Documents\Unity3D\Animal_Assets\Assets\StaticAssets\Textures\Flora\Trees\Leaves\basswood_american_2_branch_1.png"; // File Folder|png|jpg|gif|bmp
#endregion
...
unsafe void RenderInternal(Surface src)
{
    ...
    FileInfo f = new FileInfo(fileFolder);
    ...
    IEnumerable<Surface> surfaces = files.Select(fz => SurfaceFromFile(fz.FullName));

    palette = new IntelligentColorPalette();
    palette.InitializePalette(surfaces, 24, 1, 1, 254, 6, 40);

    palette.CopyPaletteToSurface(working, false);
}
...
private void AssignUIValues()
{
    ...
    fileFolder.Value = (string)Amount5;
}
...
TrackingProperty<string> fileFolder = new TrackingProperty<string>();
...
private Surface SurfaceFromFile(string filePath)
{
    using (var iStream = File.Open(filePath, FileMode.Open))
    using (var iImage = Image.FromStream(iStream))
    {
        var iSurface = Surface.CopyFromGdipImage(iImage, false);
...
public class IntelligentColorPalette
{
    ...
    public void InitializePalette(IEnumerable<Surface> surfaces, int colorSampleEfficiency,
