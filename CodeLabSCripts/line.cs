...
{ 
    ...
    Pair<float,float>[] points = new [] {
        new Pair<float, float>(.1f, .1f),
        new Pair<float, float>(.9f, .1f),
        new Pair<float, float>(.9f, .9f),
        new Pair<float, float>(.1f, .9f),
    };

    for(int i = 0; i < points.Length; i++)
    {
        Pair<float,float> point1 = points[i];
        ...
        float numerator = point2.Second - point1.Second;
        float denominator = point2.First - point1.First;
        ...
        float b = point1.Second - (m * point1.First);

        Func<int, int> line = new Func<int,int>((x) => (int)((m * x) + b));

        Debug.WriteLine(string.Format("y = ({0})x + ({1})", m, b));
