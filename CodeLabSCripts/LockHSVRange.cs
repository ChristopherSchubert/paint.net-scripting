...
IntSliderControl Amount5 = 0; // [0,360,1] Hue Minimum
IntSliderControl Amount6 = 360; // [0,360,1] Hue Maximum
CheckboxControl Amount7 = true; // [0,1] Normalize Hue
IntSliderControl Amount8 = 0; // [0,100,3] Saturation Minimum
IntSliderControl Amount9 = 100; // [0,100,3] Saturation Maximum
CheckboxControl Amount10 = false; // [0,1] Normalize Saturation
IntSliderControl Amount11 = 0; // [0,100,5] Value Minimum
IntSliderControl Amount12 = 100; // [0,100,5] Value Maximum
CheckboxControl Amount13 = false; // [0,1] Normalize Value
#endregion
...
unsafe void PreRenderInternal(Surface src)
{
    ...
    if (normalizeHue || normalizeValue || normalizeSaturation)
    {
        for (int y = selection.Top; y < selection.Bottom; y++)
        {
            ...
            for (int x = selection.Left; x < selection.Right; x++)
            {
                ...
                if (srcHsv.Value < valueLow) valueLow = srcHsv.Value;
                if (srcHsv.Value > valueHigh) valueHigh = srcHsv.Value;
...
unsafe void RenderInternal(Surface dst, Surface src)
{
    ...
    Debug.WriteLine("Value Low/Hi: " + valueLow.ToString() + "/" + valueHigh.ToString());
    ...
    for (int y = selection.Top; y < selection.Bottom; y++)
    {
        ...
        for (int x = selection.Left; x < selection.Right; x++)
        {
            ...
            if (normalizeHue)
            {
                workingHsv.Hue = (int)((((float)(workingHsv.Hue - hueLow) / (float)(hueHigh - hueLow)) * (hueMaximum - hueMinimum)) + hueMinimum);
            }
            else 
            {
                if (workingHsv.Hue < hueMinimum) workingHsv.Hue = hueMinimum;
                if (workingHsv.Hue > hueMaximum) workingHsv.Hue = hueMaximum;
            }
            
            if (normalizeSaturation)
            {
                workingHsv.Saturation = (int)((((float)(workingHsv.Saturation - satLow) / (float)(satHigh - satLow)) * (saturationMaximum - saturationMinimum)) + saturationMinimum);
            }
            else 
            {
                if (workingHsv.Saturation < saturationMinimum) workingHsv.Saturation = saturationMinimum;
                if (workingHsv.Saturation > saturationMaximum) workingHsv.Saturation = saturationMaximum;
            }

            if (normalizeValue)
            {
                workingHsv.Value = (int)((((float)(workingHsv.Value - valueLow) / (float)(valueHigh - valueLow)) * (valueMaximum - valueMinimum)) + valueMinimum);
            }
            else 
            {
                if (workingHsv.Value < valueMinimum) workingHsv.Value = valueMinimum;
                if (workingHsv.Value > valueMaximum) workingHsv.Value = valueMaximum;
            }
...
private void AssignUIValues()
{
    ...
    hueMinimum.Value = (int)Amount5;
    hueMaximum.Value = (int)Amount6;
    saturationMinimum.Value = (int)Amount8;
    saturationMaximum.Value = (int)Amount9;
    valueMinimum.Value = (int)Amount11;
    valueMaximum.Value = (int)Amount12;
    normalizeHue.Value = (bool)Amount7;
    normalizeSaturation.Value = (bool)Amount10;
    normalizeValue.Value = (bool)Amount13;
}
...
TrackingProperty<int> saturationMinimum = new TrackingProperty<int>();
TrackingProperty<int> saturationMaximum = new TrackingProperty<int>();
TrackingProperty<int> valueMinimum = new TrackingProperty<int>();
TrackingProperty<int> valueMaximum = new TrackingProperty<int>();
TrackingProperty<bool> normalizeHue = new TrackingProperty<bool>();
TrackingProperty<bool> normalizeSaturation = new TrackingProperty<bool>();
TrackingProperty<bool> normalizeValue = new TrackingProperty<bool>();
