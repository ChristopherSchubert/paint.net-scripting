...
CheckboxControl Amount6 = true; // [0,1] {!Amount3} Use Original Transparency As Guideline
IntSliderControl Amount7 = 127; // [1,255] {Amount6} Reject Alpha Below
IntSliderControl Amount8 = 3; // [1,100] Blur Radius
IntSliderControl Amount9 = 100; // [0,200] Saturation Adjustment
IntSliderControl Amount10 = 0; // [-50,50] Contrast Adjustment
IntSliderControl Amount11 = 3; // [1,100] Color Frequency Equalization
IntSliderControl Amount12 = 4; // [1,5] Color Palette Length
IntSliderControl Amount13 = 20; // [2,20] Color Palette Hue Segmentation
IntSliderControl Amount14 = 3; // [1,7] Post Processing Segments
CheckboxControl Amount15 = true; // [0,1] Post Processing Dark Is Lower
IntSliderControl Amount16 = 3; // [1,8] Post Processing Oil Painting Brush Width
CheckboxControl Amount17 = true; // [0,1] Do Post Processing Relief
ListBoxControl Amount18 = 2; // {Amount17} Relief Blend Mode|Normal|Additive|Average|Blue|Color|ColorBurn|ColorDodge|Cyan|Darken|Difference|Divide|Exclusion|Glow|GrainExtract|GrainMerge|Green|HardLight|HardMix|Hue|Lighten|LinearBurn|LinearDodge|LinearLight|Luminosity|Magenta|Multiply|Negation|Overlay|Phoenix|PinLight|Red|Reflect|Saturation|Screen|SignedDifference|SoftLight|Stamp|VividLight|Yellow
IntSliderControl Amount19 = 0; // [0,3] Final Gloss
#endregion
...
unsafe void RenderInternal(Surface src)
{
    ...
    working4.CopySurface(src);

    if (useOriginalTransparencyAsGuideline)
    {
    ...
    if (blurRadius > 0)
    {
        GaussianBlur(working2, working, selection, blurRadius);
    ...
    {
        HueAndSaturation(working, working, selection, 0, saturationAdjustment);
    }
    ...
    palette.InitializePalette(working, 1, colorFrequencyEqualization, 5, 250, colorPaletteLength, colorPaletteHueSegmentation);
    ...
    OilPainting(working, working, selection, postProcessingOilPaintingBrushWidth, 50);

    var toneBandMin = byte.MaxValue;
    var toneBandMax = byte.MinValue;
    for(var y = 0; y < working.Height; y++)
    {
        for(var x = 0; x < working.Width; x++)
        {
            ...
            var band = GetTone(working[x,y]);

            if (band < toneBandMin) toneBandMin = band;
            if (band > toneBandMax) toneBandMax = band;
        }
    }

    var toneRange = toneBandMax - toneBandMin;
    Debug.WriteLine(toneRange);
    ...
    for(var postProcessingSegmentIndex = 0; postProcessingSegmentIndex < postProcessingSegments; postProcessingSegmentIndex++)
    {
        ...
        if (!postProcessingDarkIsLower)
        {
            toneBandIdentifier = (postProcessingSegments-1)-postProcessingSegmentIndex;
        }
        
        var segmentLowToneBand = toneBandMin + (toneBandIdentifier * (toneRange / postProcessingSegments));
        var segmentHighToneBand = toneBandMin + ((1+toneBandIdentifier) * (toneRange / postProcessingSegments));

        if (postProcessingSegmentIndex == 0)
        {
            segmentLowToneBand = 0;
        }
        if (postProcessingSegmentIndex == postProcessingSegments -1)
        {
            segmentHighToneBand = 255;
        }
        ...
        if (doPostProcessingRelief)
        {
            var toneBandSurface = GetSurfaceFromToneBand(working, Convert.ToInt32(segmentLowToneBand), Convert.ToInt32(segmentHighToneBand));

            var angle = 45 + (((postProcessingSegmentIndex+1f)/(float)postProcessingSegments)*(90 - 45));
            
            Relief(working2, toneBandSurface, selection, (double)angle);

            BlendSurfaces(working2, toneBandSurface, working2, reliefBlendMode);
            ColorSwap(working2, working2, ColorBgra.Black, ColorBgra.Transparent);

            BlendSurfaces(working3, working3, working2, BlendModes.Normal);
        }
    } 
    ...
    if (finalGloss > 0)
    {
        GaussianBlur(working2, working, selection, finalGloss);
        working.CopySurface(working2);
    }
}
...
private void AssignUIValues()
{
    ...
    showPalette.Value = Amount5;
    useOriginalTransparencyAsGuideline.Value = Amount6;
    rejectAlphaBelow.Value = Amount7;
    blurRadius.Value = Amount8;
    saturationAdjustment.Value = Amount9;
    contrastAdjustment.Value = Amount10;
    colorFrequencyEqualization.Value = Amount11;
    colorPaletteLength.Value = Amount12;
    colorPaletteHueSegmentation.Value = Amount13;
    postProcessingSegments.Value = Amount14;
    postProcessingDarkIsLower.Value = Amount15;
    postProcessingOilPaintingBrushWidth.Value = Amount16;
    doPostProcessingRelief.Value = Amount17;
    reliefBlendMode.Value = (BlendModes)(int)Amount18;
    finalGloss.Value = Amount19;
...
TrackingProperty<int> blurRadius = new TrackingProperty<int>();
...
TrackingProperty<int> postProcessingSegments = new TrackingProperty<int>();
TrackingProperty<bool> postProcessingDarkIsLower = new TrackingProperty<bool>();
TrackingProperty<int> postProcessingOilPaintingBrushWidth = new TrackingProperty<int>();
TrackingProperty<bool> doPostProcessingRelief = new TrackingProperty<bool>();
TrackingProperty<BlendModes> reliefBlendMode = new TrackingProperty<BlendModes>();
TrackingProperty<int> finalGloss = new TrackingProperty<int>();
