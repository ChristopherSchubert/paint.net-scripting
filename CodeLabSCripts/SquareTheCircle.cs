...
IntSliderControl Amount5 = 127; // [1,255,4] Threshold
DoubleSliderControl Amount6 = 0; // [-1,1] Rotation
#endregion
...
{ 
    ...
    RemoveIslandsFromSurface(working, working, .1, true);

    Rotate(working2, working, selection, 180*rotation, 0, 0);
    working.CopySurface(working2);

    ColorBgra* workPtr = working.GetPointAddressUnchecked(selection.Left, selection.Top);
    ColorBgra workColor;
    ColorBgra* work2Ptr = working2.GetPointAddressUnchecked(selection.Left, selection.Top);
    ColorBgra work2Color;

    var yEdgeTop = (int)height;
    var yEdgeBottom = 0;
    var xEdgeLeft = (int)width;
    var xEdgeRight = 0;
    ...
    { 
        workPtr = working.GetPointAddressUnchecked(selection.Left, y);

        for (int x = selection.Left; x < selection.Right; x++)
        {
            workColor = *workPtr;

            if (workColor.A > threshold)
            {
                if (x < xEdgeLeft) { xEdgeLeft = x; }
                if (x > xEdgeRight) { xEdgeRight = x; }
                if (y < yEdgeTop) { yEdgeTop = y; }
                if (y > yEdgeBottom) { yEdgeBottom = y; }
            }
    ...
    Debug.WriteLine("Centering...");

    var diffL = xEdgeLeft;
    var diffR = width - xEdgeRight;   
    var diffT = yEdgeTop;
    var diffB = height - yEdgeBottom;

    var diffX = (int)(diffR - diffL);
    var diffY = (int)(diffB - diffT);

    for (int y = selection.Top; y < selection.Bottom; y++)
    { 
        workPtr = working.GetPointAddressUnchecked(selection.Left+diffX, y+diffY);
        work2Ptr = working.GetPointAddressUnchecked(selection.Left, y);
        ...
        for(int x = selection.Left; x < selection.Right; x++)
        {
            ...
            {
                *work2Ptr = *workPtr;
                //working2[x, y] = working[x+diffX,y+diffY];
            } 
    ...
    Debug.WriteLine("Centering completed...");

    var actualRadius = Math.Min(width-diffL-diffR, height-diffT-diffB) / 2;

    working.CopySurface(working2);
    working2.Clear(ColorBgra.Transparent);

    
    Debug.WriteLine("Top to bottom stretching...");

    for (int y = selection.Top; y < selection.Bottom; y++)
    {
        var slope = (y - centerY) / (float)(0 - centerX);

        var intercept = y - (slope);

        var rangeStartX = width;
        var rangeEndX = 0;

        for(var x = selection.Left; x < selection.Right; x++)
        {
            var sy = (slope*x) + intercept;

            if ((int)sy < 0 || (int)sy >= height)
            {
                continue;
            }

            if (working[x, (int)sy].A > threshold)
            {
                if (x < rangeStartX)
                {
                    rangeStartX = x;
        ...
        var rangeX = rangeEndX - rangeStartX;
        var fullRangeX = selection.Right - selection.Left;
        var fullRangeY = (height-y) - y;
        
        for(var x = selection.Left; x < selection.Right; x++)
        {
            var sy = (slope*x) + intercept;

            var targetX = (float)(rangeStartX + (rangeX * ((float)x/(float)fullRangeX)));
            var targetY = (slope*targetX) + intercept;

            int syi = (int)sy;

            if ((syi < 0) || (syi > height-1) || (targetY < 0) || (targetY > height-1))
            {
                continue;
            }

            working2[x, syi] = working.GetBilinearSample(targetX, targetY);
        }
    }

    Debug.WriteLine("Top to bottom stretching completed...");
    Debug.WriteLine("Left to right stretching...");

    for (int x = selection.Left; x < selection.Right; x++)
    {
        var slope = (0 - centerY) / (float)(x - centerX);

        var intercept = -(slope*x);

        var rangeStartY = height;
        var rangeEndY = 0;

        for(var y = selection.Top; y < selection.Bottom; y++)
        {
            var sx = (y - intercept)/slope;

            if ((int)sx < 0 || (int)sx >= width)
            {
                continue;
            }

            if (working[(int)sx, y].A > threshold)
            {
                if (y < rangeStartY)
                {
                    rangeStartY = y;
        ...
        var rangeY = rangeEndY - rangeStartY;
        var fullRangeY = selection.Bottom - selection.Top;
        var fullRangeX = (width - x) - x;
        
        for(var y = selection.Top; y < selection.Bottom; y++)
        {
            var sx = (y - intercept)/slope;

            var targetY = (float)(rangeStartY + (rangeY * ((float)y/(float)fullRangeY)));
            var targetX = (targetY - intercept)/slope;

            int sxi = (int)sx;

            if ((sxi < 0) || (sxi > width-1) || (targetX < 0) || (targetX > width-1))
            {
                continue;
            }

            working2[sxi, y] = working.GetBilinearSample(targetX, targetY);
        }
    }
    
    Debug.WriteLine("Left to right stretching completed...");
...
private void AssignUIValues()
{
    ...
    threshold.Value = (int)Amount5;
    rotation.Value = (double)Amount6;
...
TrackingProperty<int> threshold = new TrackingProperty<int>();
TrackingProperty<double> rotation = new TrackingProperty<double>();
...
RotateZoomEffect rotateEffect = null;
PropertyCollection rotateEffectProperties = null;
PropertyBasedEffectConfigToken rotateEffectConfigToken = null;
void Rotate(Surface dst, Surface src, Rectangle rect, double x, double y, double z)
{
    if (IsCancelRequested) return;
    ...
    try
    {
        ...
        {
            rotateEffect = new RotateZoomEffect();
            rotateEffectProperties = rotateEffect.CreatePropertyCollection();
            rotateEffectConfigToken = new PropertyBasedEffectConfigToken(rotateEffectProperties);
        }

        rotateEffectConfigToken.SetPropertyValue(RotateZoomEffect.PropertyNames.Tiling, false);
        rotateEffectConfigToken.SetPropertyValue(RotateZoomEffect.PropertyNames.RollAndRotate,new Tuple<double,double,double>(x,y,z));
        rotateEffect.SetRenderInfo(rotateEffectConfigToken, new RenderArgs(dst), new RenderArgs(src));
    ...
    {
        methodStopwatch.Stop();
        Debug.WriteLine("Rotate: " + methodStopwatch.ElapsedMilliseconds.ToString() + " ms");
    }
}
...
unsafe void RemoveIslandsFromSurface(Surface dst, Surface src, double islandSizeLimitPercentageArea, bool doLog)
{
    var removedPixels = 0;
    ...
    islandSizeLimitPercentageArea = DoubleUtil.Clamp(islandSizeLimitPercentageArea, 1/Math.Max(width, height), .333);

    var temp = new Surface(src.Size);
    temp.CopySurface(src);
    dst.CopySurface(src);

    var surfaceArea = width*height;
    var islandLimit = surfaceArea * islandSizeLimitPercentageArea;
    ...
    if (doLog)
    {
        Debug.WriteLine(string.Format("Removed {0} pixels in {1} islands.", removedPixels, removedIslands));
    }
...
private void RareLog(int x, int y, int howFrequent, int guesstimationOfOccurrences, string formatString, params object[] args)
{
     var random = rng.Next(0, (int)(guesstimationOfOccurrences));
    ...
    if (random < howFrequent || (howFrequent == 1 && guesstimationOfOccurrences == 1))
    {
...
{
   RareLog(x, y, howFrequent, (int)(width*height), formatString, args);
}

private void Log(int x, int y, string formatString, params object[] args)
{
    RareLog(x, y, 1, 1, formatString, args);
}
