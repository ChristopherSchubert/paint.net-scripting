...
IntSliderControl Amount7 = 96; // [0,512] Island Size Limit (^2)
CheckboxControl Amount8 = true; // [0,1] Standardize Edges
IntSliderControl Amount9 = 8; // [3,24] {Amount8} Edge Standardization Width
IntSliderControl Amount10 = 2; // [0,2] {Amount8} Edge Standardization Tolerance
DoubleSliderControl Amount11 = 1.5; // [0.5,3.5] {Amount11} Edge Standardization Allowed Range
ColorWheelControl Amount12 = ColorBgra.FromBgra(0,0,0,0); // [TransparentBlack?] Neutral Color
CheckboxControl Amount13 = false; // [0,1] Show Effect
IntSliderControl Amount14 = 5; // [1,10] Quality
#endregion
...
unsafe void RenderInternal(Surface dst, Surface src)
{
    ...
    if (destroyBridges)
    {
        for (int y = selection.Top; y < selection.Bottom; y++)
        {
            ...
            for (int x = selection.Left; x < selection.Right; x++)
            {
                ...
                if (MatchesNeutralColor(*wPtr))
                {
                    wPtr++;
                    continue;
                }
                
                var neighborLeft = x == 0 ? false : MatchesNeutralColor(working[x-1,y]) ? false : true;
                var neighborTop = y == 0 ? false : MatchesNeutralColor(working[x,y-1] )? false : true;
                var neighborRight = x == working.Width-1 ? false : MatchesNeutralColor(working[x+1,y]) ? false : true;
                var neighborBottom = y == working.Height-1 ? false : MatchesNeutralColor(working[x,y+1]) ? false : true;
                
                var neighborTopLeft = x == 0 ? false : y == 0 ? false : MatchesNeutralColor(working[x-1,y-1]) ? false : true;
                var neighborBottomLeft = x == 0 ? false : y == working.Height-1 ? false : MatchesNeutralColor(working[x-1,y+1]) ? false : true;
                var neighborTopRight = x == working.Width-1 ? false : y == 0 ? false : MatchesNeutralColor(working[x+1,y-1]) ? false : true;
                var neighborBottomRight = x == working.Width-1 ? false : y == working.Height-1 ? false : MatchesNeutralColor(working[x+1,y+1]) ? false : true;
        ...
        for (int x = selection.Left; x < selection.Right; x++)
        {
            ...
            FloodFill(x, y, working, selection, 
                (colorSet) => 
                {
                    ...
                    {
                        if (colorSet.currentColor == ColorBgra.Transparent || colorSet.currentColor == ColorBgra.TransparentBlack)
                        {
                            return false;
                        }
                    }
                    else if (colorSet.currentColor == neutralColor) 
                    {
                        return false;
                    }
    ...
    while(pixelsToRemove.Count > 0)
    {
        ...
        working[coord.First, coord.Second] = neutralColor;
        if (showEffect)
        {
            working[coord.First, coord.Second] = ColorBgra.Magenta;
        }
    }
        ...
        for (int y = selection.Top; y < selection.Bottom; y++)
        {
            ...
            for (int x = selection.Left; x < selection.Right; x++)
            {
                ...
                if (!MatchesNeutralColor(*wPtr))
                {
                    ...
                    if (MatchesNeutralColor(working[x-1,y]))
                    {
                        var newColor = StandardDeviationEdge(thisColor, x, y, edgeStandardizationWidth, 1, 0);
                    ...
                    if (MatchesNeutralColor(working[x,y-1]))
                    { 
                        var newColor = StandardDeviationEdge(thisColor, x, y, edgeStandardizationWidth, 0, 1);
...
private ColorBgra StandardDeviationEdge(ColorBgra thisColor, int x, int y, int edgeWidth, int xIteratorMultiplier, int yIteratorMultiplier)
{
    ...
    if (!skip)
    {
        ...
        var issueR = (Math.Abs(thisColor.R - avgR) > stdDevR*edgeStandardizationAllowedRange) ? 1 : 0;
        var issueG = (Math.Abs(thisColor.G - avgG) > stdDevG*edgeStandardizationAllowedRange) ? 1 : 0;
        var issueB = (Math.Abs(thisColor.B - avgB) > stdDevB*edgeStandardizationAllowedRange) ? 1 : 0;

        var issues = issueR+issueG+issueB;

        var issueLimit = edgeStandardizationTolerance;

        if (issues > issueLimit)
        {
            return neutralColor;
        }
    }
...
private bool MatchesNeutralColor(ColorBgra currentColor)
{
    if (neutralColor == ColorBgra.Transparent || neutralColor == ColorBgra.TransparentBlack)
    {
...
private void AssignUIValues()
{
    ...
    neutralColor.Value = (ColorBgra)Amount12;
    showEffect.Value = (bool)Amount13;
    quality.Value = (int)Amount14;
    destroyBridges.Value = (bool)Amount5;
    standardizeEdges.Value = (bool)Amount8;
    edgeStandardizationTolerance.Value = (int)Amount10;
    edgeStandardizationWidth.Value = (int)Amount9;
    edgeStandardizationAllowedRange.Value = (double)Amount11;
...
TrackingProperty<ColorBgra> neutralColor = new TrackingProperty<ColorBgra>();
...
TrackingProperty<bool> standardizeEdges = new TrackingProperty<bool>();
TrackingProperty<int> edgeStandardizationTolerance = new TrackingProperty<int>();
TrackingProperty<int> edgeStandardizationWidth = new TrackingProperty<int>();
TrackingProperty<double> edgeStandardizationAllowedRange = new TrackingProperty<double>();
...
CheckboxControl Amount13 = false; // [0,1] Post-Island Search
IntSliderControl Amount14 = 64; // [0,512] Post-Island Search Limit
*/
...
unsafe void InternalRenderingLoop(Surface dst, Surface src)
{
    ...
    try
    {
        ...
        {
            RenderInternal(dstWorking, dstWorking);
        }
    }
    catch (Exception x)
    {
...
public class LocalPixelCube
{
    public ColorBgra upperLeft;
    public ColorBgra upper;
    public ColorBgra upperRight;
    public ColorBgra left;
    public ColorBgra right;
    public ColorBgra bottomLeft;
    public ColorBgra bottom;
    public ColorBgra bottomRight;
    
}
private void RoundEdges(ColorBgra current, ColorBgra neutral)
{
    if (MatchesNeutralColor(*wPtr))
    {
        wPtr++;
