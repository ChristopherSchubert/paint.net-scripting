...
#region UICode
CheckboxControl Amount1 = true; // [0,1] Show Half
...
{
    working.Clear(ColorBgra.Transparent);
    ...
    if (greyscale == true)
    {
        for (int y = selection.Top; y < selection.Bottom; y++)
        {
            ...
            for (int x = selection.Left; x < selection.Right; x++)
            {
                ...
                var result = CurrentPixel.B * .21f;
                result += CurrentPixel.G *.71f;
                result += CurrentPixel.R *.071f;

                CurrentPixel = ColorBgra.FromBgra((byte)result, (byte)result, (byte)result, byte.MaxValue);
    ...
    for (var y = selection.Top + filterOffset; y < selection.Bottom - filterOffset; y++)
    {
        ColorBgra* wPtr = working.GetPointAddressUnchecked(selection.Left + filterOffset, y);

        for (var x = selection.Left + filterOffset; x < selection.Right - filterOffset; x++)
        {
            ...
            {
                ColorBgra* srcPtr = src.GetPointAddressUnchecked(x-filterOffset, y+filterY);

                for (int filterX = -filterOffset; filterX <= filterOffset; filterX++)
                {
                    ColorBgra srcColor = *srcPtr;

                    xb += (double)(srcColor.B) * xSobel[filterY + filterOffset, filterX + filterOffset];
                    xg += (double)(srcColor.G) * xSobel[filterY + filterOffset, filterX + filterOffset];
                    xr += (double)(srcColor.R) * xSobel[filterY + filterOffset, filterX + filterOffset];
                    yb += (double)(srcColor.B) * ySobel[filterY + filterOffset, filterX + filterOffset];
                    yg += (double)(srcColor.G) * ySobel[filterY + filterOffset, filterX + filterOffset];
                    yr += (double)(srcColor.R) * ySobel[filterY + filterOffset, filterX + filterOffset]; 
            ...
            *wPtr = ColorBgra.FromBgra((byte)(bt),(byte)(gt),(byte)(rt),byte.MaxValue);
...
/*
private static Bitmap ConvolutionFilter(Surface src, double[,] xkernel, double[,] ykernel, double factor = 1, int bias = 0, bool grayscale = false)
{
