...
void PreRenderInternal(Surface dst, Surface src)
{
    ...
    var workingInvalidated =
        !mainLayerCompleted ||
        reseed.HasChanged ||
        retainTransparency.HasChanged ||
        useOriginalTransparencyAsGuideline.HasChanged ||
        rejectAlphaBelow.HasChanged ||
        mainBlurRadius.HasChanged ||
        saturationAdjustment.HasChanged ||
        contrastAdjustment.HasChanged ||
        colorSampleEfficiency.HasChanged ||
        colorFrequencyEqualization.HasChanged ||
        lowColorLimit.HasChanged ||
        highColorLimit.HasChanged ||
        colorPaletteLength.HasChanged ||
        colorPaletteHueSegmentation.HasChanged ||
        oilBrushWidth.HasChanged;


    var blendJitterInvalidated = blendJitterAmount.HasChanged || useShorterEdgeForJitterSize.HasChanged;
    var blendBlurInvalidated = blendJitterInvalidated || edgeBlurRadius.HasChanged;
    var blendPaletteInvalidated = blendBlurInvalidated || workingInvalidated;
    var blendOilInvalidated = blendPaletteInvalidated || oilBrushWidth.HasChanged;
    var edgeInvalidated = !edgeLayerCompleted || blendOilInvalidated || patchBlendingIssues.HasChanged || antiAliasBlendLayer.HasChanged;

    var tenPercentWidth = (int)(selection.Width * .1f);
    var tenPercentHeight = (int)(selection.Height * .1f);
    var fivePercentWidth = (int)(selection.Width * .05f);
    var fivePercentHeight = (int)(selection.Height * .05f);
    var verticalStripeRect = new Rectangle(selectionCenterX - tenPercentWidth, 0, tenPercentWidth * 2, selection.Height);
    var horizontalStripeRect = new Rectangle(0, selectionCenterY - tenPercentHeight, selection.Width, tenPercentHeight * 2);
    var tempRect = new Rectangle(0, 0, working.Width, working.Height);
    var inner80Rect = new Rectangle(tenPercentWidth, tenPercentHeight, working.Width - (tenPercentWidth * 2), working.Height - (tenPercentHeight * 2));
    var inner90Rect = new Rectangle(fivePercentWidth, fivePercentHeight, working.Width - (fivePercentWidth * 2), working.Height - (fivePercentHeight * 2));


    if (!edgeInvalidated && !workingInvalidated)
    {
        Debug.WriteLine("Neither layer invalidated at " + System.DateTime.Now.ToString());
        return;
    }

    if (workingInvalidated)
    {
        Debug.WriteLine("Working layer invalidated at " + System.DateTime.Now.ToString());
        mainLayerCompleted = false;
        ...
        if (!retainTransparency && useOriginalTransparencyAsGuideline)
        {
            for (var y = 0; y < working.Height; y++)
            {
                for (var x = 0; x < working.Width; x++)
                {
                    ...
                    if (working[x, y].A < rejectAlphaBelow)
                    {
        ...
        {
            HueAndSaturation(working, working, tempRect, 0, saturationAdjustment);
        }

        if (contrastAdjustment != 0)
        {
            BrightnessAndContrast(working, working, tempRect, 0, contrastAdjustment);
        }

        //if (mainBlurRadius > 0)
        //{
        //    GaussianBlur(working, working, tempRect, mainBlurRadius);
        ...
        palette.ReplaceSurfaceColorsWithPalette(working, working, tempRect);

        //OilPainting(working, working, tempRect, oilBrushWidth, 50);

        mainLayerCompleted = true;
    }
    ...
    if (edgeInvalidated)
    {
        Debug.WriteLine("Blend layer layer invalidated at " + System.DateTime.Now.ToString());
        edgeLayerCompleted = false;

        var horizontalJitter = (int)(blendJitterAmount * (double)working2.Width);
        var verticalJitter = (int)(blendJitterAmount * (double)working2.Height);

        if (useShorterEdgeForJitterSize)
        {
            horizontalJitter = Math.Min(horizontalJitter, verticalJitter);
            verticalJitter = verticalJitter;
        }

        working3.CopySurface(src);

        JitterSurface(working3, working3, true, false, horizontalJitter, (int)(horizontalJitter * .1f));
        JitterSurface(working3, working3, false, true, verticalJitter, (int)(verticalJitter * .1f));
        MirrorSurface(working3, working3, true, false);
        BlendSurfaces(working2, working2, working3, 1f, BlendModes.Normal);

        working3.CopySurface(src);

        JitterSurface(working3, working3, true, false, horizontalJitter, (int)(horizontalJitter * .1f));
        JitterSurface(working3, working3, false, true, verticalJitter, (int)(verticalJitter * .1f));
        MirrorSurface(working3, working3, false, true);
        BlendSurfaces(working2, working2, working3, 0.5f, BlendModes.Overlay);

        working3.CopySurface(src);

        JitterSurface(working3, working3, true, false, horizontalJitter, (int)(horizontalJitter * .1f));
        JitterSurface(working3, working3, false, true, verticalJitter, (int)(verticalJitter * .1f));
        FullyQuarterSurfaceAroundSelectionCenter(working3, working3);
        BlendSurfaces(working2, working2, working3, 0.5f, BlendModes.Overlay);

        FullyQuarterSurfaceAroundSelectionCenter(working3, working2);
        working2.CopySurface(working3);

        //if (edgeBlurRadius > 0)
        //{
        //    GaussianBlur(working2, working2, tempRect, edgeBlurRadius);
        //    ApplyAlphaThresholdToSurface(working2, 0, 0);
        //}

        if (IsCancelRequested) return;

        palette.ReplaceSurfaceColorsWithPalette(working2, working2, tempRect);

        if (IsCancelRequested) return;

        //OilPainting(working2, working2, tempRect, oilBrushWidth, 50);
        FullyQuarterSurfaceAroundSelectionCenter(working2, working2);
        //OilPainting(working2, working2, inner90Rect, 1, 50);
        //OilPainting(working2, working2, inner80Rect, 1, 50);

        palette.ReplaceSurfaceColorsWithPalette(working2, working2, tempRect);

        if (IsCancelRequested) return;

        working3.CopySurface(working2);

        methodStopwatch.Restart();
        for (int y = 0; y < working3.Height; y++)
        {
            for (int x = 0; x < working3.Width; x++)
            {
                if (IsCancelRequested) return;

                if (x != 0 && y != 0 && x != working3.Width - 1 && y != working3.Height - 1)
                {
                    continue;
                }

                var startingColor = working3[x, y];

                if (startingColor == ColorBgra.TransparentBlack)
                {
                    continue;
                }

                var startingTone = GetTone(startingColor);

                var comparison = new Func<FloodFillColorSet, bool>((set) =>
                {
                    return set.currentColor == set.initialColor;
                });

                var fill = new Func<FloodFillColorSet, ColorBgra>((set) => ColorBgra.TransparentBlack);

                FloodFill(x, y, working3, tempRect, comparison, fill);
            }
        }
        
        methodStopwatch.Stop();
        Debug.WriteLine("FloodFill: " + methodStopwatch.ElapsedMilliseconds.ToString() + " ms");

        for (int y = 0; y < working2.Height; y++)
        {
            for (int x = 0; x < working2.Width; x++)
            {
                if (IsCancelRequested) return;

                if (working3[x, y] != ColorBgra.TransparentBlack)
                {
                    working2[x, y] = ColorBgra.TransparentBlack;
                }
                else
                {
                    if (patchBlendingIssues)
                    {
                        if (x > 0 && x < dst.Width - 1 && working2[x, 0] != working2[x, working2.Height - 1])
                        {
                            working2[x, 0] = BlendedPixel(working2[x, 0], working2[x, working2.Height - 1], BlendModes.Average);
                        }

                        if (y > 0 && y < dst.Height - 1 && working2[0, y] != working2[src.Width - 1, y])
                        {
                            working2[0, y] = BlendedPixel(working2[0, y], working2[src.Width - 1, y], BlendModes.Average);
                        }
                    }
                }
            }
        ...
        if (antiAliasBlendLayer)
        {
            GaussianBlur(working3, working2, tempRect, 1);
            working2.CopySurface(working3);
        }

        edgeLayerCompleted = true;
    }
}

bool mainLayerCompleted = false;
bool edgeLayerCompleted = false;
...
void RenderInternal(Surface dst, Surface src, Rectangle rect)
{
    for (int y = rect.Top; y < rect.Bottom; y++)
    {
        ...
        {
            var primaryColor = working[x, y];
            var edgeBlendColor = working2[x, y];

            switch (debugMode)
            {
                case 0: // Debug Mode|None|

                    if (primaryColor == edgeBlendColor || edgeBlendColor == ColorBgra.TransparentBlack)
                    {
                        dst[x, y] = primaryColor;
                    }
                    else if (
                        primaryColor.R == edgeBlendColor.R &&
                        primaryColor.G == edgeBlendColor.G &&
                        primaryColor.B == edgeBlendColor.B)
                    {
                        dst[x, y] = primaryColor;
                    }
                    else
                    {
                        dst[x, y] = BlendedPixel(primaryColor, edgeBlendColor, BlendModes.Normal);
                    }
                    break;
                case 1: // Show Base Layer
                    dst[x, y] = primaryColor;
                    break;
                case 2: // Show Blend Layer
                    dst[x, y] = edgeBlendColor;
                    break;
                case 3: // Show Blend Issues

                    var verticalIssue = false;
                    ColorBgra verticalColor = ColorBgra.Black;
                    var horizontalIssue = false;
                    ColorBgra horizontalColor = ColorBgra.Black;

                    if (x > 0 && x < dst.Width - 1 && working2[x, 0] != working2[x, src.Height - 1])
                    {
                        verticalIssue = true;

                        if (y < dst.Height * .5f)
                        {
                            verticalColor = working2[x, 0];
                        }
                        else
                        {
                            verticalColor = working2[x, src.Height - 1];
                        }
                    }

                    if (y > 0 && y < dst.Height - 1 && working2[0, y] != working2[src.Width - 1, y])
                    {
                        horizontalIssue = true;

                        if (x < dst.Width * .5f)
                        {
                            horizontalColor = working2[0, y];
                        }
                        else
                        {
                            horizontalColor = working2[src.Width - 1, y];
                        }
                    }

                    if (verticalIssue && horizontalIssue)
                    {
                        dst[x, y] = (x + y) % 20 < 10 ? horizontalColor : verticalColor;
                    }
                    else if (verticalIssue)
                    {
                        dst[x, y] = verticalColor;
                    }
                    else if (horizontalIssue)
                    {
                        dst[x, y] = horizontalColor;
                    }
                    else
                    {
                        dst[x, y] = working2[x, y];
                    }

                    break;
                case 4: // Show Base Vs Blend
                    if (working2[x, y] != ColorBgra.TransparentBlack)
                    {
                        dst[x, y] = ColorBgra.Red;
                    }
                    else
                    {
                        dst[x, y] = ColorBgra.Blue;
                    }
                    break;
            }
