...
ListBoxControl Amount5 = 1; // Hue Adjustment|Truncate|Normalize|Equalize|Justify|Segment
IntSliderControl Amount6 = 0; // [0,360,1] Hue Minimum
IntSliderControl Amount7 = 360; // [0,360,1] Hue Maximum
CheckboxControl Amount8 = false; // [0,1] Invert Hue Output
ListBoxControl Amount9 = 2; // Saturation Adjustment|Truncate|Normalize|Equalize|Justify|Segment
IntSliderControl Amount10 = 10; // [0,100,3] Saturation Minimum
IntSliderControl Amount11 = 50; // [0,100,3] Saturation Maximum
CheckboxControl Amount12 = false; // [0,1] Invert Saturation Output
ListBoxControl Amount13 = 3; // Value Adjustment|Truncate|Normalize|Equalize|Justify|Segment
IntSliderControl Amount14 = 10; // [0,100,5] Value Minimum
IntSliderControl Amount15 = 60; // [0,100,5] Value Maximum
CheckboxControl Amount16 = false; // [0,1] Invert Value Output
#endregion

private int nonTransparentPixelCount = 0;
...
private Dictionary<int,int> hueFrequencies;
private Dictionary<int,int> satFrequencies;
private Dictionary<int,int> valFrequencies;

private Dictionary<int,int> hueEqualizeFrequencyMap;
private Dictionary<int,int> satEqualizeFrequencyMap;
private Dictionary<int,int> valEqualizeFrequencyMap;

private Dictionary<int,List<Tuple<int, double>>> hueJustifyFrequencyMap;
private Dictionary<int,List<Tuple<int, double>>> satJustifyFrequencyMap;
private Dictionary<int,List<Tuple<int, double>>> valJustifyFrequencyMap;

unsafe void PreRenderInternal(Surface src)
{
    working2.CopySurface(src);
}

unsafe void RenderInternal()
{
    
    hueFrequencies = new Dictionary<int,int>();
    satFrequencies = new Dictionary<int,int>();
    valFrequencies = new Dictionary<int,int>();
    ...
    for (int y = selection.Top; y < selection.Bottom; y++)
    {
        for (int x = selection.Left; x < selection.Right; x++)
        {
            ...
            if (!hueFrequencies.ContainsKey(srcHsv.Hue)) {          hueFrequencies.Add(srcHsv.Hue, 0); }
            if (!satFrequencies.ContainsKey(srcHsv.Saturation)) {   satFrequencies.Add(srcHsv.Saturation, 0); }
            if (!valFrequencies.ContainsKey(srcHsv.Value)) {        valFrequencies.Add(srcHsv.Value, 0); }
    ...
    hueEqualizeFrequencyMap = BuildEqualizeFrequencyMap(nonTransparentPixelCount, hueLow, hueHigh, hueMinimum, hueMaximum, hueFrequencies);
    satEqualizeFrequencyMap = BuildEqualizeFrequencyMap(nonTransparentPixelCount, satLow, satHigh, satMinimum, satMaximum, satFrequencies);
    valEqualizeFrequencyMap = BuildEqualizeFrequencyMap(nonTransparentPixelCount, valLow, valHigh, valMinimum, valMaximum, valFrequencies);
    
    //return; 

    hueJustifyFrequencyMap = BuildJustifyFrequencyMap(nonTransparentPixelCount, hueLow, hueHigh, hueMinimum, hueMaximum, hueFrequencies);
    satJustifyFrequencyMap = BuildJustifyFrequencyMap(nonTransparentPixelCount, satLow, satHigh, satMinimum, satMaximum, satFrequencies);
    valJustifyFrequencyMap = BuildJustifyFrequencyMap(nonTransparentPixelCount, valLow, valHigh, valMinimum, valMaximum, valFrequencies);
    ...
    for (int y = selection.Top; y < selection.Bottom; y++)
    {
        for (int x = selection.Left; x < selection.Right; x++)
        {
            ...
            try 
            {
                newHue =  ApplyRangeAdjustmentToValue(hueAdjustment.Value, x, y, workingHsv.Hue, hueLow, hueHigh, hueMinimum, hueMaximum, hueFrequencies, hueEqualizeFrequencyMap, hueJustifyFrequencyMap);
            }
            catch (Exception ex)
            {
                Debug.WriteLine(string.Format("[{0},{1}]: Error while processing hues: {2}", x, y, ex.Message));
                Debug.WriteLine(ex);

                var builder = new System.Text.StringBuilder();

                foreach(var justification in hueJustifyFrequencyMap)
                {
                    var sourceHue = justification.Key;
                    
                    foreach(var value in justification.Value)
                    {
                        builder.AppendLine(string.Format("Hue: {0} | DstHue: {1} | Freq: {2}", sourceHue, value.Item1, value.Item2));
                    }
                }
            ...
            try 
            {
                newSat =  ApplyRangeAdjustmentToValue(satAdjustment.Value, x, y, workingHsv.Saturation,  satLow, satHigh, satMinimum, satMaximum, satFrequencies, satEqualizeFrequencyMap, satJustifyFrequencyMap);
            }
            catch (Exception ex)
            {
                Debug.WriteLine(string.Format("[{0},{1}]: Error while processing saturation: {2}", x, y, ex.Message));
                Debug.WriteLine(ex);
                return;
            }
            try 
            {
                newVal =  ApplyRangeAdjustmentToValue(valAdjustment.Value, x, y, workingHsv.Value, valLow, valHigh, valMinimum, valMaximum, valFrequencies, valEqualizeFrequencyMap, valJustifyFrequencyMap);
            }
            catch (Exception ex)
            {
                Debug.WriteLine(string.Format("[{0},{1}]: Error while processing values: {2}", x, y, ex.Message));
                Debug.WriteLine(ex);
                return;
            }

            if (newHue < hueMinimum || newHue > hueMaximum)
            {
                Debug.WriteLine(string.Format("Bad hue at [{0},{1}]: was {2}.", x, y, newHue));
                return;            }
            
            if (newSat < satMinimum || newSat > satMaximum)
            {
                Debug.WriteLine(string.Format("Bad sat at [{0},{1}]: was {2}.", x, y, newSat));
                return;
            }
            
            if (newVal < valMinimum || newVal   > valMaximum)
            {
                Debug.WriteLine(string.Format("Bad val at [{0},{1}]: was {2}.", x, y, newVal));
                return;
            }

            if (invertHueOutput) 
            { 
                newHue = hueMinimum + ((hueMaximum - hueMinimum) - ((Int32Util.Clamp(newHue, hueMinimum, hueMaximum)) - hueMinimum));
            } 

            if (invertSatOutput) 
            { 
                newSat = satMinimum + ((satMaximum - satMinimum) - ((Int32Util.Clamp(newSat, satMinimum, satMaximum)) - satMinimum));
            } 
            
            if (invertValOutput) 
            { 
                newVal = valMinimum + ((valMaximum - valMinimum) - ((Int32Util.Clamp(newVal, valMinimum, valMaximum)) - valMinimum));
            }
 
            workingHsv.Hue = Int32Util.Clamp(newHue, hueMinimum, hueMaximum); 
            workingHsv.Saturation = Int32Util.Clamp(newSat, satMinimum, satMaximum); 
            workingHsv.Value = Int32Util.Clamp(newVal, valMinimum, valMaximum); 

            working[x,y] = ColorBgra.FromColor(workingHsv.ToColor());
        }
    }
}
...
private Dictionary<int,int> BuildEqualizeFrequencyMap(int eligibleCount, int low, int high, int minimum, int maximum, Dictionary<int,int> sourceFrequencies)
{
    var results = new Dictionary<int,int>();

    var inputRange = 1 + (high - low);
    var outputRange = 1 + (maximum - minimum);

    var sliceSize = (int)((double)eligibleCount / (double)outputRange);

    var count = 0;
    var sourceIterator = 0;
    
    while(sourceIterator <= inputRange)
    {
        if (IsCancelRequested) return null;

        var sourceIndex = sourceIterator + low;

        if (!sourceFrequencies.ContainsKey(sourceIndex))
        {
            sourceIterator += 1; 
            continue;
        }

        if (!results.ContainsKey(sourceIndex))
        {
            results.Add(sourceIndex, 0);
        }

        if (IsCancelRequested) return null;

        var sum = sourceFrequencies[sourceIndex];

        count += sum;

        var destinationIndex = minimum + ((int)(count / sliceSize));

        if (count == eligibleCount || destinationIndex == maximum+1)
        {
            destinationIndex -= 1;
        }
        else if (destinationIndex == minimum-1)
        {
            destinationIndex += 1;
        }
        
        results[sourceIndex] = destinationIndex;
...
private Dictionary<int,List<Tuple<int, double>>> BuildJustifyFrequencyMap(int eligibleCount, int low, int high, int minimum, int maximum, Dictionary<int,int> sourceFrequencies)
{
    var destinationResults = new Dictionary<int, List<Tuple<int, double>>>();

    var inputRange = 1 + (high - low);
    var outputRange = 1 + (maximum - minimum);

    var fullBucketSize = (int)((double)eligibleCount / (double)outputRange);
    var lastBucketSize = fullBucketSize + (eligibleCount - (fullBucketSize * outputRange));
     
    var sourceFrequenciesCopy = new Dictionary<int,int>(sourceFrequencies);

    var currentSourceIndex = low;

    for(var currentDestinationIndex = minimum; currentDestinationIndex <= maximum; currentDestinationIndex++)   
    {
        if (IsCancelRequested) return null;

        if (currentDestinationIndex == maximum)
        {
            fullBucketSize = lastBucketSize;
        } 

        if (!sourceFrequencies.ContainsKey(currentSourceIndex))
        {
        ...
        while(currentBucketSize < fullBucketSize)
        {
            ...
            if (currentSourceIndex > 500 || currentDestinationIndex > 500)
            {
                throw new NotSupportedException("Runaway justification analysis");
            }

            var amountRemainingInBucket = fullBucketSize - currentBucketSize;

            if (!sourceFrequenciesCopy.ContainsKey(currentSourceIndex))
            {
                currentSourceIndex += 1;
                continue;
            }
    ...
    Debug.WriteLine("Finalizing results for justification.");

    var results = new Dictionary<int, List<Tuple<int, double>>>();
    ...
    for(var i = minimum; i <= maximum; i++)
    {
        ...
        foreach(var subDestinationResult in destinationResult)
        {
            ...
            if (!results.ContainsKey(freq))
            {
                results.Add(freq, new List<Tuple<int,double>>());
            } 
...
private int ApplyRangeAdjustmentToValue(RangeAdjustment rangeAdjustment, int x, int y, int value, int low, int high, int minimum, int maximum, Dictionary<int,int> frequencies, Dictionary<int,int> equalizeFrequencyMap, Dictionary<int,List<Tuple<int,double>>> justifyFrequencyMap)
{
    if (IsCancelRequested) return -1;

    switch(rangeAdjustment)
    {
        ...
        case RangeAdjustment.Justify:
            ...
            if (!justifyFrequencyMap.ContainsKey(value))
            {
                throw new NotSupportedException(string.Format("No key found for {0} @ [{1},{2}].  Original count: {3}", value, x, y,
                    frequencies[value]));
...
private void AssignUIValues()
{
    ...
    invertHueOutput.Value = Amount8;
    satAdjustment.Value = (RangeAdjustment)(int)Amount9;
    satMinimum.Value = (int)Amount10;
    satMaximum.Value = (int)Amount11;
    invertSatOutput.Value = Amount12;
    valAdjustment.Value = (RangeAdjustment)(int)Amount13;
    valMinimum.Value = (int)Amount14;
    valMaximum.Value = (int)Amount15;
    invertValOutput.Value = Amount16;
}
...
TrackingProperty<bool> invertHueOutput = new TrackingProperty<bool>();
...
TrackingProperty<bool> invertSatOutput = new TrackingProperty<bool>();
...
TrackingProperty<bool> invertValOutput = new TrackingProperty<bool>();
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
/// COMMON CODE BELOW
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
///
...
{
    InternalPreRenderingLoop(src);
    InternalRenderingLoop();
...
unsafe void InternalPreRenderingLoop(Surface src)
{
    Debug.WriteLine("");
    Debug.WriteLine("--------------------------------------------------------------------------------------------");
...
unsafe void InternalRenderingLoop()
{
