// Name:File Color Swap
// Submenu:Chris_Batch
// Author:
// Title:File Color Swap
// Version:
// Desc:
// Keywords:
// URL:
...
#region UICode
FilenameControl Amount1 = @""; // Path|pdn|png
ColorWheelControl Amount2 = ColorBgra.FromBgra(255,255,255,0); // [Transparent?] Replace
ColorWheelControl Amount3 = ColorBgra.FromBgra(0,0,0,0); // [TransparentBlack?] With
...
{ 
    ...
    foreach(var filePath in Directory.GetFiles(directory))
    {
        ...
        if (!filePath.EndsWith(".png") && !filePath.EndsWith(".pdn"))
        {
            continue;
        }
        ...
        {
            document = Document.FromStream(stream);
        }
            
        foreach(BitmapLayer layer in document.Layers)
        {
            for(var y = 0; y < layer.Height; y++)
            {
                for (var x = 0; x < layer.Width; x++)
                {
                    if (layer.Surface[x,y] == replace)
                    {
                        layer.Surface[x,y] = with;
                    }
                }
            }
        } 
        ...
        using(var output = File.Open(filePath, FileMode.OpenOrCreate))
        {
            if (filePath.EndsWith("png"))
            {
                var layer = (BitmapLayer)document.Layers[0];
                using (Bitmap bitmap = layer.Surface.CreateAliasedBitmap(true))
                {
                    SavePNG(output, bitmap);
                }
            }
            else
            {
...
private void AssignUIValues()
{
    ...
    replace.Value = (ColorBgra)Amount2;
    with.Value = (ColorBgra)Amount3;
    process.Value = (bool)Amount4;
}

TrackingProperty<string> path = new TrackingProperty<string>();
TrackingProperty<ColorBgra> replace = new TrackingProperty<ColorBgra>();
TrackingProperty<ColorBgra> with = new TrackingProperty<ColorBgra>();
