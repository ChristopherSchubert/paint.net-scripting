// Name:Alpha Mask Finalization
// Submenu:Chris
// Author:
// Title:Alpha Mask Finalization
// Version:
// Desc:
// Keywords:
// URL:
...
unsafe void PreRenderInternal(Surface src)
{
    ...
    ApplyAlphaThresholdToSurface(working, 0, 127);
    ProcessSurfaceAlpha(working, working, AlphaPixelModStyle.Alpha_Black);
}
...
{ 
    ...
    foreach(var cube in LocalizedPixelCube.BuildCubeListFromSurface(working, selection, true, 1, (color) => color.A == 255))
    {
        if (cube.DevA > cube.StdDevA)
        {
            working[cube.CenterX, cube.CenterY] = cube.AverageOpaque;
        }
    }
    
    ProcessSurfaceAlpha(working, working, AlphaPixelModStyle.Threshold_Midpoint);

    foreach(var cube in LocalizedPixelCube.BuildCubeListFromSurface(working, selection, true, 0))
    {        
        dst[cube.CenterX, cube.CenterY] = RoundEdges(cube, ColorBgra.TransparentBlack);
    }

    working.CopySurface(dst);
    ...
    RemoveIslandsFromSurface(dst, working);

    ProcessSurfaceAlpha(dst, dst, AlphaPixelModStyle.Threshold_Midpoint, AlphaPixelModStyle.Alpha_Black);
}
...
private ColorBgra RoundEdges(LocalizedPixelCube cube, ColorBgra neutralColor)
{
    ...
    else if (perpendicularSum == 3)
    {
        ...
        {
            if (!hasNeighborUpperRight && !hasNeighborLowerRight)
            {
                return  neutralColor;
            }
        }
