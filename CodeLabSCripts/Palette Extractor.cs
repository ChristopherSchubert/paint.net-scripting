...
IntSliderControl Amount2 = 10; // [1,100] Blur Radius
...
CheckboxControl Amount8 = true; // [0,1] Show Palette
...
void Render(Surface dst, Surface src, Rectangle rect)
{
    ...
    if (blurRadius > 0)
    {
        GaussianBlur(workSurface1, src, rect, blurRadius);
    }
    else 
    {
    ...
    for (int y = rect.Top; y < rect.Bottom; y += colorSampleEfficiency)
    {
        ...
        for (int x = rect.Left; x < rect.Right; x += colorSampleEfficiency)
        {
            ...
            var toneBand = GetToneBand(workSurface1[x,y]);
    ...
    var colorLists = new Dictionary<ColorBgra, List<ColorBgra>>();
    ...
    for (int y = rect.Top; y < rect.Bottom; y++)
    {
        ...
        for (int x = rect.Left; x < rect.Right; x++)
        {
            ...
            else 
            {
                ...
                workSurface1[x,y] = palette[paletteIndex];
            }
        }
    }
...
HsvColor GetRepresentativeHsvColor(ColorBgra bgra)
{
    var hsv = HsvColor.FromColor(bgra.ToColor());
...
private void OverlaySourceOneOntoSourceTwo(Surface sourceOne, Surface sourceTwo, BinaryPixelOp pixelOp)
{
    for(var y = 0; y < sourceTwo.Width; y++)
    {
        for(var x = 0; x < sourceTwo.Height; x++)
        {
            if (IsCancelRequested)
            {
                return;
            }
            sourceTwo[x,y] = pixelOp.Apply(sourceOne[x,y], sourceTwo[x,y]);
        }
    }
}
