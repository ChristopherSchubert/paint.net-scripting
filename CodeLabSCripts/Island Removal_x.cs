...
IntSliderControl Amount9 = 5; // [1,10] Quality
#endregion
    ...
    if (destroyBridges)
    {
        for (int y = selection.Top; y < selection.Bottom; y++)
        {
            ...
            for (int x = selection.Left; x < selection.Right; x++)
            {
                ...
                var neighborLeft = x == 0 ? false : working[x-1,y].Evaluate((c) => c.A == 0) ? false : true;
                var neighborTop = y == 0 ? false : working[x,y-1].Evaluate((c) => c.A == 0) ? false : true;
                var neighborRight = x == working.Width-1 ? false : working[x+1,y].Evaluate((c) => c.A == 0) ? false : true;
                var neighborBottom = y == working.Height-1 ? false : working[x,y+1].Evaluate((c) => c.A == 0) ? false : true;
                
                var neighborTopLeft = x == 0 ? false : y == 0 ? false : working[x-1,y-1].Evaluate((c) => c.A == 0) ? false : true;
                var neighborBottomLeft = x == 0 ? false : y == working.Height-1 ? false : working[x-1,y+1].Evaluate((c) => c.A == 0) ? false : true;
                var neighborTopRight = x == working.Width-1 ? false : y == 0 ? false : working[x+1,y-1].Evaluate((c) => c.A == 0) ? false : true;
                var neighborBottomRight = x == working.Width-1 ? false : y == working.Height-1 ? false : working[x+1,y+1].Evaluate((c) => c.A == 0) ? false : true;
                ...
                var neutralColor = working[x,y].ModifyColorBgra(ColorModificationChannel.Alpha, ColorModificationType.Set, 0);
...
public static class ColorBgraExtensions
{
    ...
    {  
       var channelValue = GetColorModificationChannel(color, modificationChannel);
    ...
    private static byte GetColorModificationChannel(ColorBgra color, ColorModificationChannel channel)
    {
        switch(channel)
        {   
    ...
    private static void SetColorModificationChannel(ColorBgra color, ColorModificationChannel channel, byte value)
    {
        switch(channel)
        {   
            case ColorModificationChannel.Blue:
